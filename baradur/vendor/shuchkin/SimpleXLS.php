...
/**
     ...
     */
    /* public function __construct(string $filename = false, bool $isData = false, bool $debug = false)
    {
        $this->debug = $debug;
...
//                    if ( $v['type'] === self::TYPE_RK || $v['type'] === self::TYPE_RK2 ||
    ...
    protected function _oleread($sFileName, $isData = false)
    {
        ...
        $numBigBlockDepotBlocks = $this->_getInt4d(0x2c);
        $sbdStartBlock = $this->_getInt4d(0x3c);
        $rootStartBlock = $this->_getInt4d(0x30);
        $extensionBlock = $this->_getInt4d(0x44);
        $numExtensionBlocks = $this->_getInt4d(0x48);
...
//      return ($value > 0x7FFFFFFF) ? $value - 0x100000000 : $value;
    ...
    protected function _readData($bl)
    {
        ...
        while ($block !== - 2) {
            $pos  = ( $block + 1 ) * 0x200;
            $data .= substr($this->data, $pos, 0x200);
    ...
    protected function _readPropertySets()
    {
        ...
        while ($offset < strlen($this->entry)) {
            ...
            $startBlock = ord($d[ 0x74]) | ( ord($d[ 0x74 + 1 ]) << 8 ) | ( ord($d[ 0x74 + 2 ]) << 16 ) | ( ord($d[ 0x74 + 3 ]) << 24 );
            $size       = ord($d[ 0x78]) | ( ord($d[ 0x78 + 1 ]) << 8 ) | ( ord($d[ 0x78 + 2 ]) << 16 ) | ( ord($d[ 0x78 + 3 ]) << 24 );
...
//      $code = ord($this->_data[$spos]) | ord($this->_data[$spos + 1]) << 8;
        ...
        if (( $version !== 0x600 ) && ( $version !== 0x500 )) {
            return false;
        }

        if ($substreamType !== 0x10) {
            return false;
        }
        //echo "Start parse code=".base_convert($code,10,16)." version=".base_convert($version,10,16)." substreamType=".base_convert($substreamType,10,16).""."\n";
        $spos += $length + 4;
...
//      echo 'check is date '.$xfindex.' '.$this->formatRecords['xfrecords'][$xfindex]['type']."\n";
     ...
     */
    public function createDate(int $timevalue)
    {
//      $offset = ($timeoffset===null)? date('Z') : $timeoffset * 3600;
     ...
     */
    public function setDefaultFormat(string $sFormat)
    {
