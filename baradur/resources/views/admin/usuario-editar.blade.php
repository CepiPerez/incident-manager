...
<div class="container pb-3">
    ...
    <div class="editor">
      ...
      <form action="{{ route('usuarios.update', $usuario->idT) }}" method="post" autocomplete="off">
        @csrf
        @method('put')
        ...
        <div class="row m-0">
          <div class="form-group col-md p-0 mr-0 mr-md-3">
            ...
            <input class="form-control" id="usuario" name="usuario" value="{{ $usuario->Usuario }}" autofocus>
          </div>
          <div class="form-group col-md p-0">
            <label for="nombre">Nombre y Apellido</label>
            <input class="form-control" id="nombre" name="nombre" value="{{ $usuario->nombre }}" autocomplete="false">
          </div>
        </div>
        ...
        <div class="row m-0">
          <div class="form-group col-md p-0 mr-0 mr-md-3" id="gcliente">
            ...
            <select id="cliente" name="cliente" class="form-control">
              ...
              <option value="{{$cli->codigo}}" @selected($cli->codigo==$usuario->cliente)>{{$cli->descripcion}}</option>
              @endforeach
            </select>
          </div>
