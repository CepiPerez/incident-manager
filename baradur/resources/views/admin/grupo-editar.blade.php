...
<div class="container pb-3">
    ...
    <div class="row">
      <h3 class="col pt-2">Editar grupo</h3>
    </div>
    <hr class="mb-3 mt-0">
    ...
    <div class="editor">
      ...
      <form action="{{ route('grupos.update', $grupo->codigo) }}" method="post">
        @csrf
        @method('put')
        ...
        <div id="listado" class="bg-slate p-3 mb-0">
          @if (count($grupo->miembros)>0)
            @foreach ($grupo->miembros as $user)
              <div class="card slate">
                <span style="padding: .5rem 1rem;">{{$user->nombre}}</span>
                <span class="borrar_condicion ri-lg ri-delete-bin-7-line"></span>
                <input type="hidden" name="users[]" value="{{$user->idT}}">
              </div>
            @endforeach
          @endif
