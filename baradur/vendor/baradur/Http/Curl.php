...
class Curl
{
    ...
    public function httpLogin($username, $password, $http_auth = false, $auth_type = 'any') {
        if ($http_auth) {
            $this->options[CURLOPT_HTTPAUTH] = constant('CURLAUTH_' . strtoupper($auth_type));
        }
    ...
    public function execute($url = '') {
        ...
        if ($this->request = curl_init($this->url) and is_resource($this->request)) {
            ...
            if ($set_options === false or $this->response === false) {
                ...
                $result = new HttpResponse($response, $headers, $body, $this->info['http_code'], 
                    array_search($this->http_version, $this->http_versions), '');
    ...
    {
        $message = json_encode(array($this->error_msg));
        $result = new HttpResponse($message, '', $message, 200);
        $result->error_code = $this->error_code;
        $result->error_string = $this->error_msg;
        return $result;
    }
    ...
    public function getError() {
        return ($this->error_code or $this->error_msg) ? get_class($this) . ' Class Error ' . $this->error_code . ': ' . $this->error_msg : null;
    }
