...
<div class="container mb-3">
    ...
    <table class="table">
        ...
        <tbody>
          ...
          <tr>
            <td class="d-none d-sm-table-cell" >{{$rol->id}}</td>
            <td>{{$rol->descripcion}}</td>
            <td class="text-right no-pointer" style="word-spacing:.5rem;"> 
              @if($rol->id!=1)
                <a href="{{ route('roles.edit', $rol->id) }}" class="ri-lg ri-edit-line"></a>

                <a href="#" class="ri-lg ri-delete-bin-7-line @if($rol->usuarios_count>0) disabled @endif" 
                  @if($rol->usuarios_count==0)
                    onclick="window.confirm('Esta seguro que desea eliminar el rol?')?
    ...
    {{ $roles->appends(request()->query())->links(true) }}
