...
Class PrettyDump
{
    ...
    private static function processData($subject, $ignore = array(), $depth = 1, $refChain = array())
    {
        ...
        if (is_object($subject)) 
        {
            ...
            if (self::getObjectName(get_class($subject))=='Collection')
            {
            ...
            if (self::getObjectName(get_class($subject))=='Model' && !self::$show_all)
            {
                $realsubject = array(
                    'attributes' => $subject->getAttributes()
                );

                if (count($subject->getRelations())>0) {
                    $realsubject['relations'] = $subject->getRelations();
                }
    ...
    public static function getDump($data, $full=false, $custom=array())
    {
        ...
        if (is_array($data['file'])) {
            $data['file'] = str_replace('"', "'", json_encode($data['file']));
        }

        $result = self::processData($data['data']) . '<div style="margin-top:.5rem;color:gray;">// ' . $data['file'] . "</div>";

        if(strpos($result, '<button')!==false)
        {
            $result = '<div style="margin-bottom:.5rem;"><button onclick="expandAll();">Expand all</button> 
            <button onclick="collapseAll();">Collapse all</button></div>'.$result;
        }

        echo '<div style="line-height:1.3rem;background:ghostwhite;margin:0 0 .5rem 0;padding:.5rem;
        border:1px solid lavender;font-family:monospace;font-size:13px;">'.$result.'</div>
