...
<div class="container pb-3">
    ...
    <div class="row">
      <h3 class="col pt-2">Modificar regla de asignación</h3>
    </div>
    <hr class="mb-3 mt-0">
    ...
    <div class="editor">
      ...
      <form action="{{ route('asignaciones.update', $regla->id) }}" method="post">
        @csrf
        @method('put')
        ...
        <div class="form-group">
          ...
          value="{{ $regla->descripcion }}" autofocus>
        </div>
            ...
            @if ($regla->condiciones)
              ...
              <div class="card slate">
                <div class="d-flex" style="padding: .5rem 1rem;">
                  <span class="pr-3"><b>
                    @if ($con->condicion=='cliente') Cliente
                    @elseif ($con->condicion=='area') Area
                    @elseif ($con->condicion=='modulo') Módulo
                    @elseif ($con->condicion=='tipo_incidente') Tipo de incidente
                    @endif
                  </b></span>
                  <span>
                    {{ $con->helper }} 
                  </span>
                  <input type="hidden" name="conditions[]" value="{{ $con->condicion }}">
                  <input type="hidden" name="values[]" value="{{ $con->valor }}">
                  <input type="hidden" name="text[]" value="{{ $con->helper }}">
                </div>
                <span class="borrar_condicion ri-lg ri-delete-bin-7-line"></span>
              </div>
              @endforeach
          ...
          <div class="form-group col-md pr-3 pr-md-1">
            ...
            <select id="grupo" name="grupo" class="form-control">
              @foreach ($grupos as $gr)
                <option value="{{$gr['codigo']}}" @selected($regla->grupo==$gr['codigo'])>
...
<script>
  ...
  var currentUser = '{{ $regla->usuario? $regla->usuario : 0 }}';
